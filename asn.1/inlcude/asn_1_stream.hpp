...
template <uint32_t nBits, typename uint_t = typename std::enable_if<
...
template <class _bitset_t, class Iterable = std::initializer_list<_bitset_t>>
inline _bitset_t join(Iterable const &bitsets, size_t start_bit = 0, size_t end_bit = std::string::npos)
{
    std::string byte_string;
    std::for_each(std::begin(bitsets), std::end(bitsets), [&byte_string](bitset_t &set) {
    ...
    std::move({bitset_t<byte_string.size()>(byte_string)});
}
struct data_value
{
    ...
    inline void decode(std::istream &buff)
    {
        ...
        if (id_1.has_subsequent_octets())
        {
            ...
            std::vector<bitset_t<8>> bsVect;
         auto result_bitset=   join(std::transform(std::begin(ids), std::end(ids), std::back_inserter(bsVect)
        ...
        if (ids[0].get_encoding() != Id::Encoding::Primitive)
        {
            ...
            else
            {
                pos++;
                buff.seekg(pos * sizeof(byte_t));
                std::array<byte_t, lenSize> bytes;
                buff.read(reinterpret_cast<char *>(&bytes[0]),
                          lenSize * sizeof(byte_t));
                pos += lenSize;
                std::string bitStr;
                std::for_each(std::begin(bytes), std::end(bytes), [&bitStr](byte_t &b) {
                    bitStr.append(std::bitset<8>(b).to_string());
                });
